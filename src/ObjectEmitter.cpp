...
void ZigCC::ObjectEmitter::emit(const std::unique_ptr<llvm::Module> &module, const std::string &filename, std::string &error, std::string target)
{
    if (target != "rv64") {
        ...
        pass.run(*module);
        dest.flush();
        std::system("gcc -no-pie a.o");
        // std::system("rm a.o");
    } 
    else {
        std::system("clang -target riscv64-unknown-linux-gnu -march=rv64gc -c ir.ll -o a.o");
