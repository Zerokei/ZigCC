...
std::any Visitor::visitLogicalAndExpression(ZigCCParser::LogicalAndExpressionContext *ctx)
{
    ...
    // 锛堥渶瑕佸仛閫昏緫杩愮畻鐨勬儏鍐典笅锛夊垽鏂緱鍒扮殑琛ㄨ揪寮忔槸鍚﹁兘杞寲鎴� bool 绫诲瀷
...
std::any Visitor::visitAssignmentExpression(ZigCCParser::AssignmentExpressionContext *ctx)
{
    ...
    if (auto AssignmentOperator = ctx->assignmentOperator()) {
        ...
        // 鍒ゆ柇璧嬪€煎彿鍙宠竟鐨勫彉閲忔槸鍚﹀垵濮嬪寲杩囷紙鏈夊緟鏀硅繘锛屼笉鐭ラ亾鏄摢涓彉閲忥級
        if (value == nullptr) {
...
std::any Visitor::visitSelectionStatement(ZigCCParser::SelectionStatementContext *ctx)
{
    if (ctx->If() != nullptr) {
        ...
        // 鍒ゆ柇 condition 鏄惁鍙互杞寲涓� bool 绫诲瀷
        ...
        // TODO: 娉ㄦ剰 switch 璇彞鐨勬潯浠朵笉鍏佽璧嬪€硷紝杩欏拰 if 璇彞涓嶅悓锛屽苟涓斾篃涓嶄竴瀹氶渶瑕佹槸 bool 绫诲瀷
        llvm::Value* condition = std::any_cast<llvm::Value*>(visitCondition(ctx->condition()));
        if (condition == nullptr) {
            std::cout << "Error: Condition is not a valid expression." << std::endl;
            return nullptr;
...
std::any Visitor::visitIterationStatement(ZigCCParser::IterationStatementContext *ctx)
{
    if (ctx->While() != nullptr) {
		...
		if (ctx->statement() != nullptr) {
            // TODO: 杩橀渶瑕佸鐞� break 鍜� continue 璇彞
...
{
    // 寤虹珛鍏ㄥ眬鍙橀噺鎵€鍦ㄧ殑 scope锛堟湁涓斾粎鏈変竴涓級锛屽叾 currentFunction = nullptr
    // 姝ゅ悗 scopes[0] 灏辨槸鍏ㄥ眬鐨� scope 鐨勪唬鍚嶈瘝锛屾墍鏈夎繖閲岄潰鐨勫彉閲忛兘鏄彲浠ュ湪灞€閮ㄤ娇鐢ㄧ殑锛屽悓鏃堕渶瑕佹敞鎰忚瑕嗙洊鐨勫彲鑳�
...
std::any Visitor::visitSimpleDeclaration(ZigCCParser::SimpleDeclarationContext *ctx)
{
    ...
    /* 浜嬪疄涓婏紝鎴戜滑鍦� ZigCCParser.h 涓瘡涓� context 绫讳腑鐨勫嚱鏁帮紝鏈夌殑鏄苟鍒楀叧绯伙紝鍗宠〃绀�
       鍑犵涓嶅悓鐨勭被鍨嬶紝闇€瑕佸仛 if-else 鍒嗙被澶勭悊锛屾湁鐨勬槸鍚堜綔鍏崇郴锛屽鏈嚱鏁板搴旂殑 context锛�
       瑙傚療璇硶鏍戯紝鎴戜滑杩欎竴鍑芥暟灏嗕細瀹炵幇绫讳技浜� int x, y = 0; 杩欑被琛ㄨ揪寮忕殑鍒嗙锛屽嵆鎴戜滑闇€瑕�
       鍒╃敤杩欎竴绫诲瀷鐨勫嚱鏁板皢鍏跺垎瑙ｆ垚绫诲瀷鍜屽彉閲忓悕锛堟湁鐨勫嚱鏁拌礋璐ｆ彁鍙栫被鍨嬶紝鏈夌殑鍑芥暟璐熻矗鎻愬彇鍙橀噺鍚嶏紝
       鍥犳鎴戜滑绉拌繖浜涘嚱鏁版槸鍚堜綔鍏崇郴锛屽湪璇硶鏍戜笂浣撶幇鍑哄垎鍙夌殑鐗圭偣锛夛紝骞跺湪鏍堝抚涓紑杈熺┖闂村瓨鍌紝
       鍒嗚В鐨勮繃绋嬮渶瑕佷娇鐢ㄦ洿搴曞眰鐨� visitor 鍑芥暟杩斿洖闇€瑕佺殑绫诲瀷/鍙橀噺鍚�
    */
    // TODO: 鐩墠鏆傛湭鑰冭檻涓€琛屼腑鏈変袱绉嶇被鍨嬬殑鎯呭喌锛坈onst int 涔嬬被鐨勶級
    // 褰撳墠鍙€冭檻 int x, y = 0; int x = y = 0; 杩欑鎯呭喌锛宔num 浠ュ強 class 绛夊鏉傜被鍨嬩箣鍚庡啀浣滃鐞嗭紙娣诲姞鍒嗘敮澶勭悊锛堬紵锛夛級
    // 杩樻湁寮哄埗绫诲瀷杞崲鍙互鍋氾紙鎰熻搴旇涓嶉毦锛�
    ...
    if (currentScope().currentFunction != nullptr) { // 灞€閮ㄥ彉閲忕殑鎯呭喌
        if (type == nullptr) { // 姝ゆ椂鎵剧幇鏈夌殑鍙橀噺鏄惁宸茬粡瀹氫箟杩�
        ...
        for (auto var : vars) {
            ...
            // CreateAlloca 鍑芥暟灏嗙被鍨嬩负 type 鐨勫彉閲� name 鍔犲叆鏍堝抚锛屽榻愭柟寮� nullptr锛堝彲鑳� struct 绫诲瀷鏈夌敤锛燂級
            auto alloca = builder.CreateAlloca(type, nullptr, std::get<0>(var));
            // 褰撹繘琛屼簡鍒濆鍖栨椂锛孋reateStore 鍑芥暟灏嗚祴鍊肩殑琛ㄨ揪寮忓瓨鍏ヤ笂涓€姝ュ紑杈熺殑鍦板潃绌洪棿 alloca 涓�
        ...
        if (type == nullptr) { // 娉ㄦ剰鍏ㄥ眬鍙橀噺涓嶅厑璁稿嚭鐜板湪鍏ㄥ眬 scope 涓祴鍊�
...
{
    // 鏄剧劧鏄笉鑳借繖涔堝啓浠ｇ爜鐨勶紝浣嗘槸鐩墠鍙€冭檻鍙湁涓€涓� declSpecifier 鐨勬儏鍐�
...
std::any Visitor::visitFunctionDefinition(ZigCCParser::FunctionDefinitionContext *ctx)
{
    ...
    // 鍔犲叆鍒� scopes 浣滀负鎺ヤ笅鏉� body 涓殑浣滅敤鍩�
    ...
    // 娣诲姞鍙傛暟鍒楄〃涓殑鍙傛暟鍒� var_list 涓�
    // NOTE: 鍙傛暟鍒楄〃涓殑鍙傛暟锛岃涓烘槸鍏堝墠娌℃湁澹版槑杩囩殑灞€閮ㄥ彉閲�
    //      锛堜笉闇€瑕佹鏌� scope 涓槸鍚﹀凡缁忔湁鍚屽悕鍙橀噺锛�
    for (const auto& param: params) {
        std::string param_name = param.first;
        llvm::Type *param_type = param.second;
        auto alloca = this->builder.CreateAlloca(param_type, nullptr, param_name);
        // NOTE: 鍒濇瀹炵幇鐨勬槸涓嶅仛鍒濆鍖栧弬鏁�
    ...
    // 鎶涘嚭褰撳墠 scope锛屽紑濮嬪垎鏋愬叏灞€ / 涓嬩竴涓嚱鏁颁綋
